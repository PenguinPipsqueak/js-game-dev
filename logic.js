...
// Declare our variable for our 2D array, score, row, and column;
let board;
let score = 0;
let rows = 4;
let columns = 4;
...
function setGame(){
	...
	// inner kiio wukk be executed to finish before outer loop
	for(let r=0; r<rows; r++){
		for(let c=0; c < columns; c++){
			...
			//  2-3
			//  "+" is use to conactenate values if dealing with String.
			tile.id = r.toString() + "-" + c.toString();

			// get the number from the board
			// where in the board is currently set to 0
			let num = board[r][c];
...
// event that triggeres when a webpage finished loading.
...
// "e" represemts the event object, which contains information about the event occured.
function handleSlide(e){
	...
	if(["ArrowLeft", "ArrowRight", "ArrowUp", "ArrowDown"].includes(e.code)){
		...
		// (=) assigntment operator (to assign/change value of a variable), (==) operator to compare if values from left to right are equal.
		if(e.code == "ArrowLeft"){
			slideLeft();
			setTwo();
		}
		...
		}, 100); //delay time in miliseconds// use setTimeout to delay the alert
	}
}
...
function slideLeft(){
	for (let r = 0; r<rows; r++){
		...
		// slide() function will return a new value for a specific row. (mergin of tiles)
		row = slide(row);
...
function slideUp(){
	for (let c = 0; c<columns; c++){
		...
		// create a temporary array col that represents the column from top to buttom
...
// add a random "2" tile in the game board.
function setTwo(){
	...
	while(!found){
		...
		// Math.floor() - rounds down to the nearest intenger.
	...
	for(let r = 0; r<rows; r++){
		for(let c=0; c<columns; c++){
			...
			if(board[r][c] == 2048 && is2048Exist == false){
				...
				alert("Victory! You have return 8192! You are incredibly awesome!")
				is8192Exist = true;
			}
		}
	}
...
function hasLost(){
	for (let r = 0; r<rows; r++){
		for(let c = 0; c<columns; c++){
			...
			// check if the ajacent (up, down, left, right) cells have possibke merge
			if(
...
// event listener to check where you touch the scree and prevents scrolling. input targets any element that includes the word tile.
